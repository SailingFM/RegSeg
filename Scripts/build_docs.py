...
_info_fname = pjoin(os.path.dirname(__file__), 'pyacwereg', 'info.py')
INFO_VARS = {}
...
##########################################################################
# Distutils Command class for installing pyacwereg to a temporary location.
...
class TempInstall(Command):
    ...
    def run(self):
        """ build and install pyacwereg in a temporary location. """
        ...
        # Horrible trick to reload pyacwereg with our temporary instal
        for key in sys.modules.keys():
            if key.startswith('pyacwereg'):
